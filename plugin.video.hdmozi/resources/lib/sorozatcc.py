...
def configure(base_url=None, addon_handle=None, profile_dir=None, action_prefix=None, embed_resolver=None, source_classifier=None):
    global BASE_URL, ADDON_HANDLE, PROFILE_DIR, SEARCHES_FILE, ACTION_PREFIX, EMBED_RESOLVER, SOURCE_CLASSIFIER
...
def extract_snapshot_links(snapshot_data):
    ...
    def walk(node):
        if isinstance(node, dict):
            if node.get("link"):
                results.append({
                    "url": normalize_url(node.get("link").replace("\\/", "/")),
...
def classify_source_url(url):
    if SOURCE_CLASSIFIER:
        return SOURCE_CLASSIFIER(url)
    if not url:
        return {"kind": "unknown", "label": "Ismeretlen", "playable_hint": False}
    if any(marker in url.lower() for marker in [".mp4", ".m3u8", ".mpd"]):
        return {"kind": "direct", "label": "Direkt", "playable_hint": True}
    return {"kind": "embed", "label": "Embed", "playable_hint": True}


def extract_effect_links(html_text):
    results = []
    if not html_text:
        return results
    ...
    for pattern in patterns:
        for match in re.findall(pattern, html_text):
            ...
            source_info = classify_source_url(url)
            if source_info.get("playable_hint"):
                results.append({
                    "url": url,
                    "quality": "?",
                    "language": "?",
                    "storage": source_info.get("host") or "?",
                    "source_info": source_info,
                })
    return results


def merge_episode_sources(snapshot_links, effect_links):
    merged = []
    seen = set()
    for item in snapshot_links + effect_links:
        url = normalize_url(item.get("url", ""))
        if not url or url in seen:
            continue
        seen.add(url)
        source_info = item.get("source_info") or classify_source_url(url)
        merged.append({
            "url": url,
            "quality": item.get("quality") or "?",
            "language": item.get("language") or "?",
            "storage": item.get("storage") or source_info.get("host") or "?",
            "source_info": source_info,
        })
    return merged


def build_episode_label(index, source):
    parts = [str(index)]
    for value in [source.get("quality"), source.get("language"), source.get("storage")]:
        if value and value != "?":
            parts.append(value)
    source_label = source.get("source_info", {}).get("label")
    if source_label:
...
def list_episode_links(page_url, episode_index, episode_id, series_id):
    try:
        ...
        html_links = extract_effect_links(effects.get("html", ""))
        sources = merge_episode_sources(links, html_links)

        for idx, source in enumerate(sources, 1):
            add_playable_item(build_episode_label(idx, source), {"action": "play_embed", "embed_url": source["url"]})

        if not sources:
            raise ValueError("Ehhez az epizódhoz nem találtam lejátszható forrást")
    except Exception as exc:
...
def play_embed(embed_url):
    try:
        resolved = EMBED_RESOLVER(embed_url) if EMBED_RESOLVER else resolve_embed_url(embed_url)
        ...
        item_path = resolved_url
        if headers:
            item_path = resolved_url + "|" + build_header_string(headers)
