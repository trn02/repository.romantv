...
def iter_mix_columns(data, matrix):
    for i in (0, 4, 8, 12):
        for row in matrix:
            mixed = 0
            for j in range(4):
                if data[i + j] == 0 or row[j] == 0:
                    value = 0
                else:
                    value = RIJNDAEL_EXP_TABLE[(RIJNDAEL_LOG_TABLE[data[i + j]] + RIJNDAEL_LOG_TABLE[row[j]]) % 0xFF]
                mixed ^= value
            yield mixed
...
def decrypt_cbc(ciphertext, key, iv):
    plain = bytes(aes_cbc_decrypt(list(ciphertext), list(key), list(iv)))
